...
Quick recap of last week's navigation — `pwd`, `ls`, `ls -l`, `ls -la`, `cd`, `cd ..`, `cd ~`, `cat`, `nano`, `sudo`. Then pull down today's data — a real-ish web-server **access log** (~300 lines) and a small **CSV** — into a fresh folder:
