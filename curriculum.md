...
**EK Business Academy Copenhagen** · 30 sessions × 3 hours
...
*This is the rhythm of the architecture sessions (6–22). The **IT-infrastructure foundations block**
...
| **Gitea** (pinned `v1.26.2`) | The **systems-half anchor** — a real server-side, multi-user, data-backed system | Sessions 11, 12–15, 20 |
| `ek-ita-swa-examples` | **Runnable** minimal examples (`docker compose up`) | Sessions 8, 9, 15 |
...
**Sessions 6–22**; the exam project is **Sessions 23–30**. (Session 1 is a 3rd-semester intro placeholder.)
...
| 14 | **Event-driven architecture** | Gitea's queue (one port, three brokers) and the event fan-out to webhooks; queues vs pub/sub, delivery & ordering guarantees. |
| 15 | **Microservices vs monoliths** | Gitea as a real *modular monolith*; run the same product as a monolith vs **polyglot** microservices (Kotlin + Python + Go). Failure isolation; the distributed-monolith trap. |
| **16–19** | **Project: small distributed system** | *No teaching.* Build a modular monolith, applying Sessions 12–15. |
...
| 22 | **Synthesis: real-world architectures** | Read two real architectures with the whole semester's vocabulary and weigh their trade-offs. |
| **23–30** | **Exam project** | *No teaching.* Design and partially implement a system of your choice; present and defend it at the exam. |
...
- **Mid-semester project (16–19)** — a small distributed system, built as a **modular monolith** (not microservices — Session 15 is the *why*).
- **Exam project (23–30)** — the assessment. You design and partially implement an architecture for a system of your choice, then present and defend it. The exam exercises every concept introduced across the semester.
