...
- Bring your S14 data-architecture deliverable.
...
### Part 0 — Compare notes from S14 (10 min)
Pairs swap S14 deliverables: one data-model decision that held up, one that was shaky. Two pairs share.
...
In Gitea: that cached commit count is wrong the moment someone pushes. *When* and *how* does it get invalidated? That's a **push event** triggering invalidation — a direct bridge to next week (S16): **invalidation is an event.**
...
The thread to S16: every cache is a bet that the source won't change before the TTL — and keeping a cache correct is fundamentally about **reacting to events** (a write happened; invalidate). Event-driven architecture, next week, is that idea generalised.
...
Bring it to session 16. First 10 minutes we compare.
