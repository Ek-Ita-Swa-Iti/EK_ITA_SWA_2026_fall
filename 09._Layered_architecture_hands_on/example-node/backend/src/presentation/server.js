...
// It talks only to the application layer below it — never to persistence.

const http = require("http");
const notesService = require("../application/notesService");
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/notes") {
    return send(res, 200, await notesService.findAll());
  }
  ...
  if (req.method === "POST" && url.pathname === "/notes") {
    ...
    const note = await notesService.create(body.title, body.body);
    return send(res, 201, note);
  }

  const singleNote = url.pathname.match(/^\/notes\/(\d+)$/);
  if (req.method === "GET" && singleNote) {
    const note = await notesService.findById(Number(singleNote[1]));
